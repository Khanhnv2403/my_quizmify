...
const MCQ = ({ game }: Props) => {
  ...
  }, [checkAnswer, toast, status, questionIndex, game.questions.length]);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      switch (event.key) {
        ...
        case "Enter":
          ...
          break;
        case "ArrowDown":
          setSelectedChoice((prev) => (prev + 1) % 4);
          break;
        case "ArrowUp":
          setSelectedChoice((prev) => (prev === 0 ? 3 : prev - 1));
          break;
        default:
          break;
      }
    ...
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:w-[80vw] max-w-4xl w-[90vw]">
      ...
      <Card className="w-full mt-4">
        <CardHeader className="flex flex-row items-center">
          <CardTitle className="mr-5 text-center divide-y divide-zinc-600/50 leading-tight">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-start">
                ...
                <div className="flex-start">{option}</div>
              </div>
            </Button>
          );
        })}
